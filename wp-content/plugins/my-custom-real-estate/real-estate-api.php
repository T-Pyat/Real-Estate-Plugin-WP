...
function re_api_create_estate($request) {
    ...
    if (empty($params['title'])) {
        return new WP_Error('missing_title', __('Поле "title" є обовʼязковим', 'realestate'), ['status' => 400]);
    }
    ...
    if (is_wp_error($post_id)) {
        return new WP_Error('insert_failed', __('Не вдалося створити обʼєкт', 'realestate'), ['status' => 500]);
    }
...
function re_api_update_estate($request) {
    ...
    if (!$post || $post->post_type !== 'estate') {
        return new WP_Error('not_found', __('Обʼєкт не знайдено', 'realestate'), ['status' => 404]);
    }
...
function re_api_delete_estate($request) {
    ...
    $post = get_post($id);
    if (!$post || $post->post_type !== 'estate') {
        return new WP_Error('not_found', 'Обʼєкт не знайдено', ['status' => 404]);
    }
...
function re_api_import_xml($request) {
    ...
    if (empty($url) || !filter_var($url, FILTER_VALIDATE_URL)) {
        return new WP_Error('invalid_url', __('Недійсний або відсутній URL XML', 'realestate'), ['status' => 400]);

    }

    $response = wp_remote_get($url);
    if (is_wp_error($response)) {
        return new WP_Error('fetch_failed', __('Не вдалося отримати XML', 'realestate'), ['status' => 500]);
    }
    
    $body = wp_remote_retrieve_body($response);
    if (empty($body)) {
        return new WP_Error('empty_body', __('Порожній вміст XML', 'realestate'), ['status' => 500]);
    }
    ...
    if ($xml === false) {
        ...
        return new WP_Error('invalid_xml', __('Невалідний XML формат', 'realestate'), ['status' => 500]);
    }    
